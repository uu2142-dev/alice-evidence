...
*(v5.1 — v5 plus a fourth live mechanism, a public deployment anyone can audit (§5.6d), and corrected implementation-status language in §3.4 and §10 — the post-quantum signing and IPFS pinning are design targets, not yet code, and this version says so. v5 added three live mechanisms beyond the financial gate; see §5.6. v4 was revised after the system falsified one of the author's central claims; see §5.)*
...
[07] MERKLE SEAL          SHA-256 chain; every stage hashed
```
...
Every pipeline execution produces a Merkle tree where each leaf is a SHA-256 hash of one stage's output; the root seals the complete exchange. Tamper-evident: any post-hoc modification changes the root. Stored locally, no required vendor egress. **Implementation status, stated exactly:** the SHA-256 content-and-linkage chains are implemented and independently verifiable (`memory/archivist.py`, `memory/verify_chain.py` in the linked repo); the public deployment (§5.6d) additionally signs each root with Ed25519, making exported records origin-attestable; post-quantum SPHINCS+-256 signatures and IPFS pinning are the enterprise design target and are **not yet implemented** — I state this so no reader has to grep the repo to discover it. In personal deployment the ledger carries inference records, resonance records, and evolutionary records — an auditable record of the relationship neither party can deny or alter.
...
### 5.6 Generalizing the mechanism — four further live demonstrations (knowledge, biometrics, record, public deployment)

§5 showed the gate working in the one domain with clean external ground truth. Four further mechanisms now run live, each at a deliberately *different* epistemic tier — stated plainly so the weaker ones are not mistaken for the stronger.
...
**(c) A running, verifiable, correctable-not-erasable record.** The Archivist of §3.4 now exists as a running, independently-verifiable instance (a SHA-256 content-and-linkage hash chain — the implemented local form). Its `verify()` reconstructs the chain and detects tampering *even when a modified entry is re-hashed* — the alteration surfaces at the next link. And a design commitment, tested: growth is **correctable, not erasable.** Mistakes are not deleted; they are superseded by sealed corrections, leaving both the error and its correction in the record. (As the operator puts it: we fall as we learn to walk — the falls are the lesson.)

**(d) A public deployment anyone can audit.** Since July 12, 2026, a public instance of the gate architecture runs at **www.rabbitholeai.ai**: four model families behind one pipeline, every answer returning a cost-plus receipt (real provider token counts × rates pinned from primary pricing pages, the full margin disclosed), the validated bias screen of §3.2 as a labeled triage signal — with the deployed build's own held-out validation metrics published live at `rabbitholeai.ai/api/bias/health` (re-validated on fresh data pulls, July 2026: toxicity 0.924 ± 0.007, framing 0.842) — and a SHA-256 Merkle seal over each exchange, Ed25519-signed by the gate. Sessions live in the visitor's own browser (there is no server-side conversation database) and export as JSON that standard tooling can verify. The sealed archive is also *functional* memory: relevant past exchanges are recalled as context only after the gate re-verifies their signature and re-hashes their content against the sealed leaves. In a live adversarial test, a forged memory carrying a valid signature but altered text was rejected, and the model honestly reported not knowing. This is the mechanism of (c) — a record that defends its own integrity — running in public, where any reader of this post can test it.

Together these extend the central claim — a verifiable mechanism for refusing to act on confident falsehood — from one domain into general knowledge, personal grounding, the integrity of memory itself, and a public deployment. None is the co-evolution proof (§6 remains pending); all are runnable in the linked repository or usable live at rabbitholeai.ai.
...
- The post-quantum signing (SPHINCS+) and IPFS pinning described for the enterprise Archivist are design targets, not implemented code; what runs today is SHA-256 chaining plus Ed25519 root-signing in the public deployment (§3.4, §5.6d).
...
- The public deployment's bias screen (§5.6d) is a labeled triage signal for toxicity and framing style — it is not a truth or hallucination detector, and its interface says so on every response.
...
5. Help me specify the minimal falsifiable test of the *co-evolutionary* claim. The code is at github.com/uu2142-dev/alice-evidence, and the public gate is live at rabbitholeai.ai for anyone who wants to test the sealed-session claims directly.
...
MIT-licensed Python stack. Full source: github.com/uu2142-dev/alice-evidence. Simulation Merkle roots sealed March 5–14, 2026. Live demonstration artifacts: pre-registered proposals `58d89a19…`, verdict + data-integrity metadata `3b3c9b13…`. BiasScorer: MiniLM (all-MiniLM-L6-v2) sentence embeddings → MLP classifier, two heads (toxicity, framing-bias); validated leakage-free with held-out 5-fold and official-split evaluation — toxicity AUROC 0.923±0.009 (civil_comments), framing-bias AUROC 0.840 official-split / 0.857±0.013 5-fold (BABE); in-sample 1.000 on both (memorization baseline, reported to show the held-out gap); independently re-validated on fresh data pulls July 2026 (0.924±0.007 / 0.842), with the deployed build's metrics served live at rabbitholeai.ai/api/bias/health. ResonanceScorer: cosine vs commitment vector, 16-sample rolling window. NEAT: pure numpy/scipy, custom speciation + historical marking. MerkleArchivist: SHA-256 chained (implemented — `memory/archivist.py`, `memory/verify_chain.py`); the public deployment signs roots with Ed25519 (public key served by the site's API); SPHINCS+-256 signing and IPFS pinning are the enterprise design target, not yet implemented. Target edge hardware: Radxa Rock 5B 32GB + NVMe + solar (~$1,300) — assembly now in progress; the live deployments to date run on commodity cloud instances. Built with Claude, Grok, Gemini, and GPT-4o across iterative sessions; the dual-head resonance architecture was developed collaboratively. Naming this explicitly because presenting it as a solo achievement would be inaccurate.
